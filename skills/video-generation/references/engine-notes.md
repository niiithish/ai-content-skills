...
- **Aspect ratio.** State it explicitly in OUTPUT SETTINGS. Engines that default to 16:9 will hand back landscape for a vertical ad unless told otherwise. The ratio trap is specifically about the **opening frame**: when a keyframe seeds the generation, the output follows that frame's shape, so a landscape keyframe on a vertical ad needs regenerating at 9:16 rather than arguing with it in text. A wide *location* reference is a different thing and stays wide — it is read for spatial understanding rather than used as frame one, and a 16:9 environment sheet gives the model far more geometry to work with than a cropped vertical version of the same room.
