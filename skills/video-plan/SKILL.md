...
5. **Story and shots.** Write the Story paragraph, then the shot table.
   ...
   - Shot ids: a scene with one shot is just its number (`3`); a scene with several shots gets a letter each (`1a`, `1b`). Number scenes from 1 in story order.
...
6. **Fill the gaps.** Where the brief only gives voiceover, decide what we show and set it in *italic* so the user can see it's a suggestion.
   ...
   - Say who is where, what they're doing and what changes by the end of the shot, so every shot can start from a still: characters already in place, nothing growing on screen, nobody walking in through a wall.
   - Make it move. Open each "What we show" with the camera in bold (**Wide.**, **Low tracking shot.**, **Close-up.**, **Over his shoulder.**, **Her POV.**) and change it from the shot before. Every shot has something happen: someone grabs, falls, turns, reacts. Characters just sitting and looking is a deliberate pause, never two shots in a row. Aim for something new on screen every 2–5 s.
