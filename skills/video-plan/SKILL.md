...
1. **Folders.** `python3 <skill-dir>/scripts/plan.py init <client-folder> video-1` creates `brief/`, `video-1/PLAN.md` and `video-1/voiceover/`, and never overwrites anything. **Standalone videos:** some clients send unrelated projects (a photoreal b-roll library one month, an animated ad the next). Then each video folder is its own project with its own `AGENTS.md`, `brief/` and sheets, and you set it up in place with `.` as the video: `plan.py init <client>/video-2 .` puts `PLAN.md`, `brief/` and `voiceover/` straight into `video-2/`, never a `video-2/video-1/`. Use this whenever the video folder already has its own brief or `AGENTS.md`, or the client folder has no `AGENTS.md` at its top. If the brief came as a chat message, save it verbatim to `brief/message-YYYY-MM-DD.md`. Read every file in `brief/` in full, including the editing and caption sections.
   - **PDFs:** convert each one once, next to it: `pdf2md --raw "brief/<name>.pdf" > "brief/<name>.md"`. From then on read the `.md` (skip the conversion if it already exists), in full. Never plan from a partial read, and never ask the user what the brief says: reread it.
   - **Reference or inspiration videos:** run the `video-breakdown` skill on each one before planning (you can't watch video; Gemini can). A breakdown JSON someone saved earlier is only a hint until it passes that skill's check. Write the reference's pattern into the plan: its hook, how each segment opens and builds, each character's own world, how often something new happens. The client sent it because they want that.
...
4. **Time the voiceover.**
   - Copy the script to `video-N/voiceover/script.md`, one spoken line per row, in story order, word for word from the brief's `.md` (or the client's message). Every sentence of the script is in it; leave nothing out because it looks like a note.
...
5. **Story and shots.** Write the Story paragraph, then the shot table.
   ...
   - Clips are 4, 6, 8 or 10 s. Each shot is one visible beat.
   - **Lines spoken on camera** stay whole: one sentence is never split across two clips (the model cuts it off). A line up to about 9.5 s gets one clip that ends 0.5–2 s after the last word. A longer line gets the **fewest** clips that hold it, each up to 10 s, split at a sentence or clause break: a 16–18 s line is two clips, not three. Those clips chain (each starts from the last frame of the one before) so the line plays as one continuous shot.
...
6. **Fill the gaps.** Where the brief only gives voiceover, decide what we show and set it in *italic* so the user can see it's a suggestion.
   ...
   - Make it move. Open each "What we show" with the camera in bold (**Wide.**, **Low angle.**, **Close-up.**, **Over his shoulder.**, **Her POV.**) and change it from the shot before. These are framings, not moves: clips are generated with a locked camera, so the motion comes from the characters. (Clips where a character talks to camera are the exception: they get one slow, smooth move, a push-in, drift or arc, never a crash zoom or whip.) A POV shows what that character sees, so they aren't in it (at most a paw or a nose at the frame edge). Two shots of the same characters in the same spot need clearly different cameras (wide from behind them, then a close-up of one face), or the stills come out alike. Every shot has something happen: someone grabs, falls, turns, reacts. Characters just sitting and looking is a deliberate pause, never two shots in a row. Aim for something new on screen every 2–5 s. Write each shot's visual idea into the plan, not just its framing: what appears, grows or changes, and on which words. The first drafts should already be engaging; "a character stands and talks" is a boring clip.
